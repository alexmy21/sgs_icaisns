...
SGS 的一个重要部分是管理元数据。这意味着同时处理原始数据和关于数据的数据。文章介绍了一种叫 HllSets 的数据结构，它基于 HyperLogLog 算法。这个结构旨在高效分析数据，同时遵循集合论原则，为 HllSet 关系代数奠定基础。这种代数包括多个 HllSets 和如并集、交集、补集等操作。
